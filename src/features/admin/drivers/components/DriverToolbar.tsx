// src/features/admin/drivers/components/DriverToolbar.tsx
"use client";

import { Search, Plus, Bike, Car, LayoutGrid } from "lucide-react";
import { cn } from "@/utils/cn";

interface DriverToolbarProps {
  search: string;
  onSearchChange: (val: string) => void;
  filter: "all" | "motor" | "mobil";
  onFilterChange: (val: "all" | "motor" | "mobil") => void;
  onAdd: () => void;
}

export default function DriverToolbar({ search, onSearchChange, filter, onFilterChange, onAdd }: DriverToolbarProps) {
      ...
      <div className="flex p-1 bg-white rounded-full shadow-sm border border-surface-200">
         {filters.map((item) => {
            ...
            return (
              <button
                key={item.value}
                onClick={() => onFilterChange(item.value as "all" | "motor" | "mobil")}
                className={cn(
